...
resource "null_resource" "docker_push" {
  provisioner "local-exec" {
    command = <<EOT
      ...
      docker tag ${var.image_name}:${var.image_version_tag} $(aws ecr describe-repositories --repository-names ${var.ecr_repository_name} --region ${var.aws_region} | jq -r '.repositories[0].repositoryUri'):latest
